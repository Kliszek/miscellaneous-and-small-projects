...
//#define int long long
{
   ...
   //cout<<"Wszystkie karty: \n";
    ...
    for(int i=1; i<=parkart; i++)
    {
        D[i].push_back(make_pair(drzewo[i],2));
        //cout<<"  karta nr "<<i<<"\n";//<<"DEBUG  "<<D[i].size()<<"\n";;
    }
    ...
    for(int i=parkart/2; i>0; i--)
    {
        ...
        {
            D[(D[i*2][j].second+2)/2-1] = make_pair(D[i*2][j].first+a, D[i*2][j].second+2);
        }
        ...
        for(int j=0; j<D[i*2+1].size(); j++)
        {
            D[(D[i*2+1][j].second+2)/2-1]=make_pair(D[i*2+1][j].first+a, D[i*2+1][j].second+2);
        }
